...
/// Digest type of the trees this expands.
pub type Digest = [u32; DIGEST];
...
/// `sibling_hashes`, and `depth` is `log2` of the domain.
///
...
pub fn expand(
    ...
    for &i in indices {
        if depth < usize::BITS as usize && i >= 1usize << depth {
            return Err(Error::IndexOutOfRange { index: i, depth });
        }
    }
    ...
    for level in 0..depth {
        ...
        while i < nodes.len() {
            ...
            parents.push((group_start / 2, poseidon2::reference::compress(&left, &right), members));
